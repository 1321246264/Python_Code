...
hive_database = const.HIVE_DATABASE
hive_database_temp = const.HIVE_DATABASE_TEMP
hive_etl_task_list = const.HIVE_ETL_TASK_LIST
hive_bmnc_devops = const.HIVE_BMNC_DEVOPS
...
def tempToFormalDataTable(dst_file_path, hive_database_temp, csvFileName, tableName, tableDate, TempTableSuffix,
    ...
    for col, type, a, b in zip(cols, types, rangea, rangeb):
        ...
        #     # insert_sql_list = insert_sql_list + 'ROW_NUMBER() OVER() AS `REC_SEQ`'  # 暂用这个,测试其他方法中（如替换方法149、150行需修改）
    ...
    # 创建临时库
    create_temp_database = '''CREATE DATABASE IF NOT EXISTS %s''' % (hive_database_temp)
    ...
    create_temp_table_sql = '''CREATE TABLE IF NOT EXISTS %s.%s (%s) ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.OpenCSVSerde' with SERDEPROPERTIES ("separatorChar"=",","quotechar"="\\"") STORED AS TEXTFILE''' % (
        hive_database_temp, tableName + tableDate.replace('-', ''), tablist.strip(','))
    create_temp_near_line_table_sql = '''CREATE TABLE IF NOT EXISTS %s.%s (%s) ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.OpenCSVSerde' with SERDEPROPERTIES ("separatorChar"=",","quotechar"="\\"") STORED AS TEXTFILE''' % (
        hive_database_temp, tableName + TempTableSuffix, tablist.strip(','))
    ...
    if task_type == 'z' and interfaceId == '120036':  # 任务类型为 LX 则为离线数据 且为增量数据 接口ID为 120036
        z_create_temp_table_sql = '''CREATE EXTERNAL TABLE IF NOT EXISTS %s.%s (%s) ROW FORMAT SERDE 'org.apache.hadoop.hive.serde2.OpenCSVSerde' with SERDEPROPERTIES ("separatorChar"=",","quotechar"="\\"") STORED AS TEXTFILE location '%s' ''' % (
        hive_database_temp, tableName + tableDate.replace('-', ''), tablist.strip(','), hdfs_path + '/' + tableName + tableDate.replace('-', ''))
        hiveConnect.execute(z_create_temp_table_sql)
        upload_hive(dst_file_path, tableName + tableDate.replace('-', ''), hdfs_path, csvFileName, interfaceId)
        ...
        # 本地文件 load 至 支持 csv 格式加载的临时表中
        upload_hive(dst_file_path, tableName + TempTableSuffix, hdfs_path, csvFileName, interfaceId)
    # 原Data_Dt字段作为分区字段,如存在则需在insert语句中剔除
    orc_table_cols = ','.join(
        filter(lambda x: x != ' CAST(`DATA_DT` AS DATE)', insert_sql_list.strip().strip(',').upper().split(',')))
    # print(orc_table_cols)
    # print('************')
    # print(orc_table_cols.strip().strip(','))
    insert_orc_sql = '''INSERT OVERWRITE TABLE %s.%s PARTITION(DATA_DT = '%s') SELECT %s FROM %s.%s''' % (
        hive_database, tableName, tableDate, orc_table_cols, hive_database_temp, tableName + tableDate.replace('-', ''))
    insert_near_line_orc_sql = '''INSERT OVERWRITE TABLE %s.%s PARTITION(DATA_DT = '%s',File_DtimeSeq = '%s') SELECT %s FROM %s.%s''' % (
        hive_database,tableName, tableDate, File_DtimeSeq, orc_table_cols, hive_database_temp, tableName + TempTableSuffix)
    if task_type == 'z' and interfaceId == '120036':  # 任务类型为 LX 则为离线数据 且为增量数据 接口ID为 120036
        orc_table_cols = orc_table_cols.replace('CAST(`REC_SEQ` AS INT)', 'ROW_NUMBER() OVER(ORDER BY REC_ID)')
        # orc_table_cols = orc_table_cols.replace('DEFAULT.ROW_SEQUENCE()', 'ROW_NUMBER() OVER(ORDER BY REC_ID)')
        # orc_table_cols = orc_table_cols.replace('ROW_NUMBER() OVER()', 'ROW_NUMBER() OVER(ORDER BY REC_ID)')
        insert_orc_sql = '''INSERT OVERWRITE TABLE %s.%s PARTITION(DATA_DT = '%s') SELECT %s FROM %s.%s''' % (
        hive_database, tableName, tableDate, orc_table_cols, hive_database_temp, tableName + tableDate.replace('-', ''))
...
def upload_hive(dst_file_path, tableName, hdfs_path, csvFileName, interfaceId):
    ...
    if interfaceId == '120036':   # 任务类型为 LX 则为离线数据 且为增量数据 接口ID为 120036 # hdfs对应目录上文件已存在则不上传
        put_file = 'hdfs dfs -put -f %s %s' % (dst_file_path, hdfs_path + '/' + tableName)
        log.info(put_file)
        os.system(put_file)  # 上传至主机时取消此注释
    else:
        log.info(put_file)
        os.system(put_file)  # 上传至主机时取消此注释
        load_hive = '''load data inpath '%s' overwrite into table %s.%s''' % (put_path, hive_database_temp, tableName)
...
def HivePartitionRecordNumber(tableName, tableDate, File_DtimeSeq, category):
    ...
    countSql = '''SELECT COUNT(*) FROM %s.%s WHERE DATA_DT = '%s' ''' % (hive_database, tableName, tableDate)
    nearLineCountSql = '''SELECT COUNT(*) FROM %s.%s WHERE DATA_DT = '%s' AND File_DtimeSeq = '%s' ''' % (
        hive_database, tableName, tableDate, File_DtimeSeq)
    ...
    log.info('tableCount: ' + str(countlist[0]))
    return countlist[0]  # 返回记录条数


def HiveDropTempTable(hive_database_temp, tableName, TempTableSuffix, category, tableDate):  # 清除临时表
    """
    删除csv文件加载至hive正式表过程中产生的临时表
    :param hive_database_temp: hive临时表所在库名
    ...
    """
    # hiveConnect = self.CreateConnectHive()
    dropTempTable = '''DROP TABLE %s.%s''' % (hive_database_temp, tableName + tableDate.replace('-', ''))
    dropNearLineTempTable = '''DROP TABLE %s.%s''' % (hive_database_temp, tableName + TempTableSuffix)
    if category == 'JX':  # 任务类型为 JX 则为近线数据
...
def HiveInsertEtlLog(tableSystem, tableName, tableDate, tableDesc, csvFileName, processTime, File_DtimeSeq, category):
    ...
    if category == 'JX':  # 任务类型为 JX 则为近线数据
        insertHiveEtlLogSql = '''
        INSERT INTO %s.%s PARTITION(DATA_DT = '%s')
        SELECT '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', from_unixtime(unix_timestamp(),"yyyy-MM-dd HH:mm:ss"), '%s'
        ''' % (
            hive_bmnc_devops, hive_etl_task_list ,tableDate, tableSystem, tableName, tableDesc, csvFileName.split('_')[1], 'JX', csvFileName,
        ...
        SELECT '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', from_unixtime(unix_timestamp(),"yyyy-MM-dd HH:mm:ss"), '%s'
        ''' % (
            hive_bmnc_devops, hive_etl_task_list, tableDate, tableSystem, tableName, tableDesc, csvFileName.split('_')[1], 'LX', '', csvFileName,
            processTime,
            time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), os.path.basename(__file__))
