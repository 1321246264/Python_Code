...
def searchInterfaceCodeData(InterfaceCode):
    ...
    """
    searchData_sql = '''
    SELECT etl_system, etl_job, category, description, interfaceCode, tableName 
    FROM %s.etl_job 
    WHERE TRIM(INTERFACECODE) = '%s'
    ''' % (MysqlDatabase, InterfaceCode)
    # 使用 cursor 方法创建一个游标
    log.info(searchData_sql)
    cursor = mysql_connect().cursor()
    cursor.execute(searchData_sql)
    result = cursor.fetchone()
    etl_system, etl_job, category, description, interfaceCode, tableName = result[0], result[1], result[2], result[3], \
                                                                           result[4], result[5]
    return etl_system, etl_job, category, description, interfaceCode, tableName


def searchTableInformationData(TableName):
    searchData_sql = '''
    SELECT DatabaseName,TableName,columnInfo
    FROM %s.table_information
    WHERE TableName = '%s'
    AND Valid = 0
...
def InsertUpdateLog(FtpFilePath, FtpFileName, FtpFileSize, FtpFileRecordNumber, FtpFilePushTime, HiveTableName,
    ...
    if etl_job_status == 'assign':  # mysql日志插入,任务分配
        ...
        ''' % (
            MysqlDatabase, FtpFilePath, FtpFileName, FtpFileSize, FtpFileRecordNumber, FtpFilePushTime, etl_job_status,
            etl_begin_time)
    ...
    # mysql日志更新,csv文件下载至本地l
        ...
        update_sql = '''
        UPDATE %s.etl_job_log
        SET etl_job_status='%s', etl_job_error_log='%s', downloadTime='%s', pid='%s'
        WHERE FtpFileName = '%s'
        ''' % (
            MysqlDatabase, etl_job_status, etl_job_error_log, time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()), pid,
            FtpFileName)
    # mysql日志更新,csv文件加载至hive
        ...
        WHERE FtpFileName = '%s'
        ''' % (
            MysqlDatabase, HiveTableName, HiveLoadPartition, HivePartitionRecordNumber, etl_job_status,
            etl_job_error_log,
    ...
    # mysql日志更新,完成
        ...
        update_sql = '''
        UPDATE %s.etl_job_log
        SET HiveTableName='%s', HiveLoadPartition='%s', HivePartitionRecordNumber=%d, etl_job_status='%s', etl_job_error_log='%s', hdfsToHiveTime='%s', pid='%s'
        WHERE FtpFileName = '%s'
        ''' % (
            MysqlDatabase, HiveTableName, HiveLoadPartition, HivePartitionRecordNumber, etl_job_status,
            etl_job_error_log,
