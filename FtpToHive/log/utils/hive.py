...
def tempToFormalDataTable(dst_file_path, hive_database, csvFileName, tableName, tableDate, TempTableSuffix,
                          File_DtimeSeq, hdfs_path, category):
    # 获取table字典
    ...
    if category == 'JX':  # 任务类型为 JX 则为近线数据
        ...
        upload_hive(dst_file_path, tableName + TempTableSuffix, hdfs_path, csvFileName)
    elif category == 'LX':  # 任务类型为 LX 则为离线数据
        hiveConnect.execute(create_temp_table_sql)
        # 本地文件 load 至 支持 csv 格式加载的临时表中
        upload_hive(dst_file_path, tableName, hdfs_path, csvFileName)
    ...
    insert_near_line_orc_sql = '''INSERT OVERWRITE TABLE BMNC_SDATA.%s PARTITION(DATA_DT = '%s',File_DtimeSeq = '%s') SELECT %s FROM %s.%s''' % (
        tableName, tableDate, orc_table_cols, File_DtimeSeq, hive_database, tableName + TempTableSuffix)
    # 开启hive动态分区
...
def upload_hive(dst_file_path, tableName, hdfs_path, csvFileName):
    """
    ...
    # load_hive = '''load data inpath '%s' overwrite into table %s.%s''' % (put_path, hive_database, tableName) # 加载后本地文件会被删除
    load_hive = '''load data local inpath '%s' overwrite into table %s.%s''' % (dst_file_path, hive_database, tableName) # 加载后本地文件不会被删除
