...
def beeline_exec_sql(sql_list, eror_message):
    ...
    for sql in sql_list:
        exec_command += '''-e 'set hive.exec.dynamic.partition.mode=nonstrict;set hive.exec.max.dynamic.partitions.pernode=10000;set hive.exec.max.dynamic.partitions=10000;set hive.exec.max.created.files=10000;%s' ''' % sql
...
def sqoop_load_data_for_bigtab():
    ...
    for dbtab in tableList:
        ...
        # 原Data_Dt字段，Data_Stat_Std_Cd字段作为分区字段需在建表sql中提出Data_Dt字段和Data_Stat_Std_Cd字段
        orc_table_cols = ','.join(filter(lambda x: x.strip().split(
            ' ')[0] != '`Data_Dt`', tablist.strip(',').split(',')))
        orc_table_cols = ','.join(filter(lambda x : x.strip().split(
            ' ')[0] != '`Data_Stat_Std_Cd`',orc_table_cols.strip(',').split(',')))

        create_orc_sql = '''CREATE TABLE IF NOT EXISTS %s.%s(%s) PARTITIONED BY (Data_Dt string,Data_Stat_Std_Cd string) STORED AS ORC tblproperties ("orc.compress" = "SNAPPY") ''' % (
            database, tabname, orc_table_cols)
        beeline_exec_sql([create_orc_sql], 'Table [%s.%s] create failed!' % (database, tabname))
        ...
        for date in dayrange:
            ...
            # orc表添加分区
            # add_part_sql = '''ALTER TABLE %s.%s ADD PARTITION(Data_Dt="%s",Data_Stat_Std_Cd="01")''' % (database, tabname, startDate)
            # add_part_sql_02 = '''ALTER TABLE %s.%s ADD PARTITION(Data_Dt="%s",Data_Stat_Std_Cd="02")''' % (database, tabname, startDate)
            # beeline_exec_sql([add_part_sql], 'Table [%s.%s] already has partition [%s]!' % (database, tabname, startDate))
            # beeline_exec_sql([add_part_sql_02], 'Table [%s.%s] already has partition [%s]!' % (database, tabname, startDate))
            ...
            INSERT OVERWRITE TABLE %s.%s PARTITION(Data_Dt,Data_Stat_Std_Cd) SELECT %s ,Data_Dt,Data_Stat_Std_Cd  FROM %s WHERE to_date("%s") <= to_date(%s) AND to_date(%s) < to_date("%s") ''' % (
                database, tabname, sqlList.strip(','), tmptab, startDate, splitkey, splitkey, endDate)

            # tmp_to_orc = '''INSERT OVERWRITE TABLE %s.%s PARTITION(Data_Dt="%s",Data_Stat_Std_Cd="01") SELECT %s FROM %s WHERE to_date("%s") <= to_date(%s) AND to_date(%s) < to_date("%s") AND Data_Stat_Std_Cd = '01' ''' % (
            #     database, tabname, startDate, sqlList.strip(','), tmptab, startDate, splitkey, splitkey, endDate)
            # tmp_to_orc_02 = '''INSERT OVERWRITE TABLE %s.%s PARTITION(Data_Dt="%s",Data_Stat_Std_Cd="02") SELECT %s FROM %s WHERE to_date("%s") <= to_date(%s) AND to_date(%s) < to_date("%s") AND Data_Stat_Std_Cd = '02' ''' % (
            #     database, tabname, startDate, sqlList.strip(','), tmptab, startDate, splitkey, splitkey, endDate)
            log.info(tmp_to_orc)
            beeline_exec_sql([tmp_to_orc], 'Faild sync TMP to ORC table [%s.%s]!' % (database, tabname))
            # beeline_exec_sql([tmp_to_orc_02], 'Faild sync TMP to ORC table [%s.%s]!' % (database, tabname))
